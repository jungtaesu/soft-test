...
버그마다 아래를 서술해 주세요:
- 어느 줄에서 발생하는가? validate 함수 안
- 어떤 상황에서 문제가 생기는가? 
- 수정된 코드: 

- 어느 줄에서 발생하는가? handleChange 함수 안
- 어떤 상황에서 문제가 생기는가? 연속 호출/ 동시 업데이트 시 간헐적 버그
- 수정된 코드:

- 어느 줄에서 발생하는가? handleSubmit 함수 안
- 어떤 상황에서 문제가 생기는가? 이벤트를 props로 받지 않고 바로 실행시키는 에러 
- 수정된 코드:
